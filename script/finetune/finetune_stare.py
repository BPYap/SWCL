...
from retinal_fundus_encoder.dataset import STAREDataset
...
def main():
    ...
    callback = STARECallback()
    model = SegmentationModel(n_channels=3, num_classes=2)
    train_dataset = None
    if args.train_image_dir and args.train_mask_dir:
        train_dataset = STAREDataset(args.train_image_dir, args.train_mask_dir)
    eval_dataset = None
    if args.eval_image_dir and args.eval_mask_dir:
        eval_dataset = STAREDataset(args.eval_image_dir, args.eval_mask_dir)
